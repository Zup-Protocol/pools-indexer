...
import { ZebraProtocolV3Factory_PoolCreated_event } from "generated";
import { MockDb, ZebraProtocolV3Factory } from "generated/src/TestHelpers.gen";
...
describe("ZebraProtocolV3Factory", () => {
  const mockDb = MockDb.createMockDb();
  let network: IndexerNetwork;
  let event: ZebraProtocolV3Factory_PoolCreated_event;
  ...
  beforeEach(() => {
    ...
    event = ZebraProtocolV3Factory.PoolCreated.createMockEvent({
      mockEventData: {
        chainId: network,
      },
  ...
  it("should pass the correct protocol when calling the pool created handler", async () => {
    ...
    assert.equal(passedProtocol, SupportedProtocol.ZEBRA_V3);
  });
