...
import { ProjectXV3Factory_PoolCreated_event } from "generated";
import { MockDb, ProjectXV3Factory } from "generated/src/TestHelpers.gen";
...
describe("ProjectXV3Factory", () => {
  const mockDb = MockDb.createMockDb();
  let network: IndexerNetwork;
  let event: ProjectXV3Factory_PoolCreated_event;
  ...
  beforeEach(() => {
    ...
    event = ProjectXV3Factory.PoolCreated.createMockEvent({
      mockEventData: {
        chainId: network,
      },
  ...
  it("should pass the correct protocol when calling the pool created handler", async () => {
    ...
    assert.equal(passedProtocol, SupportedProtocol.PROJECT_X_V3);
  });
